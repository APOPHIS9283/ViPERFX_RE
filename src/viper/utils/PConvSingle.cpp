...
void PConvSingle::ConvSegment(float *buffer, bool interleaved, int channel)
{
    ...
    pffft_transform(this->fftSetup, this->fftBuffer,
                    this->inputHistory[this->delayLineIndex],
                    this->fftWork, PFFFT_FORWARD);
    ...
    for (int k = 0; k < this->segmentCount; k++)
    {
        ...
        pffft_zconvolve_accumulate(this->fftSetup,
    ...
    // Inverse FFT
    pffft_transform(this->fftSetup, this->accumBuffer, this->fftBuffer,
                    this->fftWork, PFFFT_BACKWARD);
...
int PConvSingle::LoadKernel(const float *kernel, float gain, int kernelSize, int segmentSize)
{
    if (kernel != nullptr && kernelSize >= 2 && segmentSize >= 2 && (segmentSize & (segmentSize - 1)) == 0)
    {
        this->instanceUsable = false;
...
int PConvSingle::ProcessKernel(const float *kernel, int kernelSize, int unused)
{
    ...
    this->overlapBuffer = (float *)pffft_aligned_malloc(this->segmentSize * sizeof(float));
    this->monoBuffer = (float *)pffft_aligned_malloc(this->segmentSize * sizeof(float));

    if (!this->fftWork || !this->fftBuffer || !this->accumBuffer ||
        !this->overlapBuffer || !this->monoBuffer)
    {
        return 0;
    }
    ...
    {
        this->filterSegments[i] = (float *)pffft_aligned_malloc(this->fftSize * sizeof(float));
        this->inputHistory[i] = (float *)pffft_aligned_malloc(this->fftSize * sizeof(float));
    ...
    for (int i = 0; i < this->segmentCount; i++)
    {
        ...
        pffft_transform(this->fftSetup, this->fftBuffer, this->filterSegments[i],
                        this->fftWork, PFFFT_FORWARD);
    }
...
int PConvSingle::ProcessKernel(const float *kernel, float gain, int kernelSize, int unused)
{
